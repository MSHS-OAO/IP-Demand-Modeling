...
covid_steady_state_census_add <- covid_census_steady_state %>%
  ...
  mutate(Scenario = "COVID Steady State") %>%
  ungroup()
...
covid_ss_flu_census_add$Scenario <- factor(covid_ss_flu_census_add$Scenario,
                                           levels = c("Flu Surge", "COVID Steady State"), ordered = TRUE)
...
census_additions_table <- function(site) {
  ...
  kable(census_add_site, format = "html", escape = FALSE, align = "c", digits = 1,
      ...
      caption = paste(site, " Census Addition Considerations")) %>%
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11, full_width = FALSE) %>%
...
**ICUs at each site do not always see a drastic  increase in cesus during flu months.**
...
**Moving forward, use difference between lowest non-flu month and highest flu month as flu surge census.**
