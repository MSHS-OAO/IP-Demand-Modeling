...
#### Dashboard creation date: `r Sys.Date()`
...
covid_ip_days_df_raw <- read.csv(paste0(user_directory, "/DataMart/COVID_IP_PATIENT_DAYS_DETAIL_202007081143.csv"), stringsAsFactors = FALSE, na = c("", "NA"))


# Import Epic daily COVID census
epic_covid_census_raw <- read_excel(paste0(user_directory, "/Epic COVID Repo/COVID MRN Daily Census Export 2020-07-08.xlsx"))
...
# Import flu test data from Epic
...
# 
# Import analysis reference for discharge data
...
# Import analysis reference for Epic data
epic_reference_file <- paste0(user_directory, "/Flu Test Data/Analysis Reference 2020-07-08.xlsx")

epic_site_ref <- read_excel(epic_reference_file, sheet = "Epic Site Lookup")
...
# Remove encounters with sites as UNDETERMINED
dm_covid_days_df <- dm_covid_days_df %>%
  filter(IP_SITE != "UNDETERMINED")
...
dm_covid_days_df <- dm_covid_days_df %>%
  mutate(DataMartICUDay = (IP_SITE == "MSSN" & (CENSUS_BED_TYPE %in% "Critical Care" | CENSUS_BED_TYPE %in% "Critical Care Surge")) |
         ...
         COVIDCensusDay = PAT_DAY_INF_STATUS %in% c("COVID-19", "PUI - COVID", "SUSC COVID"))
...
dm_covid_census_subset <- dm_covid_days_df %>%
  ...
  select(IP_SITE, CSN, MRN, 
         ...
         DataMartICUDay, LOAD_DATE, INFECTION_STATUS, PAT_DAY_INF_STATUS, COVIDCensusDay) %>%
  mutate(DataMartInfection = PAT_DAY_INF_STATUS)

dm_covid_census_subset$INFECTION_STATUS <- NULL
dm_covid_census_subset$PAT_DAY_INF_STATUS <- NULL
...
epic_covid_census_subset <- epic_covid_census %>%
  filter(ED_Loc == FALSE & !is.na(Site)) %>%
...
```{r Format SEIR census estimates and potential peaks}
...
# Subset data for July/Aug peak (reintroduction rate and date of 0.017 2020-04-23)
...
# Set sites as factors
system_summer_peak$Site <- factor(system_summer_peak$Site, levels = site_order, ordered = TRUE)
system_summer_peak <- system_summer_peak %>%
  arrange(Site)

```

```{r COVID steady state for subtraction}
# Create dataframe of COVID steady state census to subtract from SEIR model peak and historical peak scenarios
...
```{r Update SEIR model based peak based on steady state census}
seir_peaks <- left_join(system_summer_peak, covid_steady_state_subtract, by = c("Site" = "Site"))
  
seir_peaks_summary <- seir_peaks %>%
  mutate(TotalPeakBeyondSS = SiteTotalPeak - SSIPADC,
         ICUPeakBeyondSS = SiteICUPeak - SSCriticalCareADC,
         Scenario = "DOM Predictive Model")

seir_peaks_summary <- seir_peaks_summary %>%
  select(Site, TotalPeakBeyondSS, ICUPeakBeyondSS, Scenario)

```

```{r Scale historical peak COVID IP census data}

# Scale ICU census at peak
scale_factors_covid <- data.frame("Scale" = c(0.25, 0.5))

# Calculate historical peak COVID scenarios using the difference between peak census and steady state census
# covid_icu_scenarios <- covid_census_icu_peak_summary %>%
#   select(Site, CriticalCareCensus)
covid_icu_hist_peak <- covid_census_icu_peak_summary %>%
...
covid_icu_hist_peak <- left_join(covid_icu_hist_peak,
                                 covid_steady_state_subtract,
                                  by = c("Site" = "Site"))

covid_icu_hist_peak <- covid_icu_hist_peak %>%
  mutate(PeakBeyondSSIPADC = PeakIPCensus - SSIPADC,
         PeakBeyondSSCriticalCareADC = PeakCriticalCareCensus - SSCriticalCareADC)

covid_icu_hist_peak[ , c("SSIPADC", "SSCriticalCareADC")] <- NULL
...
# Determine lengths to replicate data frames for looking at historical % scenarios
scale_length_covid <- nrow(scale_factors_covid)
covid_icu_length <- nrow(covid_icu_hist_peak)

# Replicate data frames to 
...
covid_icu_hist_scenarios_scale <- covid_icu_hist_peak %>%
  select(Site, PeakBeyondSSIPADC, PeakBeyondSSCriticalCareADC)
covid_icu_hist_scenarios_scale <- covid_icu_hist_scenarios_scale[rep(rownames(covid_icu_hist_scenarios_scale), scale_length_covid), ]
covid_icu_hist_scenarios_scale = cbind(covid_icu_hist_scenarios_scale, Scale = scale_factors_covid)

# Calculate census for different scenarios based on historical data and scale
covid_icu_hist_scenarios_scale <- covid_icu_hist_scenarios_scale %>%
  mutate(ScaledCriticalCareCensus = PeakBeyondSSCriticalCareADC * Scale,
         Scenario = paste0(Scale*100, "% Historical Peak"))
...
```{r Combine SEIR scenario and historical peak scenarios}

covid_icu_hist_scenarios_summary <- covid_icu_hist_scenarios_scale %>%
  select(Site, ScaledCriticalCareCensus, Scenario)

colnames(covid_icu_hist_scenarios_summary) <- c("Site", "CriticalCareCensus", "Scenario")

seir_scenario_summary <- seir_peaks_summary %>%
  select(Site, ICUPeakBeyondSS, Scenario)

colnames(seir_scenario_summary) <- c("Site", "CriticalCareCensus", "Scenario")

covid_icu_scenario_summary <- rbind(covid_icu_hist_scenarios_summary, seir_scenario_summary)

covid_icu_scenario_summary$Scenario <- factor(covid_icu_scenario_summary$Scenario,
                                              levels = c("DOM Predictive Model", "25% Historical Peak", "50% Historical Peak"),
                                              ordered = TRUE)

covid_icu_scenario_summary$Metric <- "Critical Care Census"

covid_icu_scenario_summary_cast <- dcast(covid_icu_scenario_summary, Site + Metric ~ Scenario, value.var = "CriticalCareCensus")


# # Set site and scale name to factors before casting
# covid_icu_hist_scenarios_scale$ScaleName <- factor(covid_icu_scenarios_scale$ScaleName,
#                                    levels = c("25% Peak", "50% Peak"), ordered = TRUE)
# 
...
```{r Custom function for creating COVID-19 historical census}
covid_hist_census_peak <- function(site) {
  site <- site
  ...
  covid_icu_hist_site <- covid_icu_hist_peak %>%
    filter(Site == site)

  covid_icu_hist_site$Site <- NULL
  
  kable(covid_icu_hist_site, format = "html", escape = FALSE, align = "c", digits = 1,
      col.names =c("Date", "IP Census", "Critical Care Census", "IP Census", "Critical Care Census"),
      caption = paste(site, " COVID-19 Historical Census")) %>%
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11, full_width = FALSE) %>%
  column_spec(column = c(3), border_right = "thin solid lightgray") %>%
  ...
  column_spec(column = c(1:5), background = "inherit", color = "inherit") %>%
  # row_spec(row = 0, background = "#221f72", color = "white") %>%
  add_header_above(c("COVID-19 Historical Peak Census" = 3, "COVID-19 Peak Census Beyond Steady State" = 2), background = c(sinai_colors[1], sinai_colors[2]), color = "white", line = FALSE)
}
...
covid_census_project <- function(site) {
  ...
  covid_disch_scenarios_site <- covid_icu_scenario_summary_cast %>%
    filter(Site == site)
  ...
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11, full_width = FALSE) %>%
  column_spec(column = c(1), border_right = "thin solid lightgray") %>%
  column_spec(column = c(2:4), background = sinai_colors[1], color = "white", include_thead =  TRUE) %>%
  # column_spec(column = c(5:6), background = sinai_colors[2], color = "white", include_thead =  TRUE) %>%
  column_spec(column = c(1:4), background = "inherit", color = "inherit") %>%
  # row_spec(row = 0, background = "#221f72", color = "white") %>%
  add_header_above(c(" ", "Scenario 1:" = 1, "Scenario 2:" = 1, "Scenario 3:" = 1), background = c("white", sinai_colors[1], sinai_colors[1], sinai_colors[1]), color = "white", line = FALSE)
}

```

```{r Preprocess Epic flu data for Epic sites}
...
# Crosswalk sites
epic_sites_flu_df <- left_join(epic_sites_flu_df, epic_site_ref, by = c("Location" = "Location"))
...
```{r Preprocess Epic flu data for MSBI}
...
flu_test_volume <- ggplot(data = site_monthly_flu_volume_melt, aes(x = OrderMonthYear, y = value, na.rm = TRUE)) +
...
flu_positive_volume <- ggplot(data = site_monthly_test_results_melt, aes(x = OrderMonthYear, y = value, na.rm = TRUE)) +
  geom_line(aes(color = Site)) +
  geom_point(aes(shape = Site, color = Site)) +
...
ip_disch_epic_cw_subset <- ip_disch_df_raw %>%
...
epic_ip_disch_crosswalk <- left_join(system_flu_crosswalk, ip_disch_epic_cw_subset, by = c("VisitNumber" = "VisitNumber"))

epic_ip_disch_crosswalk$IPAdmission <- !is.na(epic_ip_disch_crosswalk$`Encounter No`)

# Test to see percentage of patients classified as IP in Epic who have IP billing charges
ip_billing_check <- epic_ip_disch_crosswalk %>%
  group_by(Site, PtClass) %>%
  summarize(IPBill = sum(IPAdmission),
            NoIPBill = sum(!IPAdmission),
            IPBillPercent = IPBill / n(),
            NoIPBillPercent = NoIPBill / n())

```


```{r Determine and graph patients admitted inpatient with positive flu tests}
...
admitted_flu_positive_volume <- ggplot(data = flu_positive_admits_melt, aes(x = OrderMonthYear, y = value, na.rm = TRUE)) +
  geom_line(aes(color = Site)) +
  geom_point(aes(shape = Site, color = Site)) +
...
flu_positive_census_kable <- kable(flu_positive_ip_days_summary, format = "html", escape = FALSE, align = "c", 
...
```{r Create visualizations of basic output of SEIR model}
# Plot MSHS overall census predictions
seir_system_overall_census <- ggplot(data = seir_census_df, aes(x = Date, y = TotalCensus, color = Model)) +
  geom_line() + 
...
# Plot MSHS ICU/Vent census predictions
seir_system_icu_census <- ggplot(data = seir_census_df, aes(x = Date, y = ICUCensus, color = Model)) +
  geom_line() + 
...
# Create table summarizing secondary peak census
seir_sec_peak_table <- summer_peak %>%
  select(PeakCensus, PeakICUCensus) %>%
  mutate(Scenario = "Summer Peak Census Projections")

seir_sec_peak_table <- seir_sec_peak_table[ , c("Scenario", "PeakCensus", "PeakICUCensus")]

seir_sec_peak_kable <- kable(seir_sec_peak_table, format = "html", escape = FALSE, align = "c", digits = 1,
      col.names =c("Scenario", "IP Census", "Critical Care Census"),
      caption = "Predictive Model Secondary Peak Census Projections") %>%
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11, full_width = FALSE) %>%
  column_spec(column = c(1), border_right = "thin solid lightgray", background = "white") %>%
  column_spec(column = c(2:3), background = sinai_colors[1], color = "white", include_thead =  TRUE) %>%
  column_spec(column = c(1:3), background = "inherit", color = "inherit")
...
seir_adm_prob <- ggplot(data = seir_admit_prob_df) +
...
# Create kable to visualize admissions probability by site
avg_prob_admit_table <- avg_prob_admit %>%
  mutate(ProbPercent = paste0(round(AvgProb, 1), "%"))

avg_prob_admit_table$AvgProb <- NULL

adm_prob_kable <- kable(avg_prob_admit_table, format = "html", escape = FALSE, align = "c", digits = 1,
      col.names =c("Site", "Probability of Admission"),
      caption = "Probability of Admission by Site") %>%
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11, full_width = FALSE) %>%
  column_spec(column = c(1), border_right = "thin solid lightgray", background = "white") %>%
  column_spec(column = c(2), background = sinai_colors[1], color = "white", include_thead =  TRUE) %>%

  column_spec(column = c(1:2), background = "inherit", color = "inherit") #%>%
  # add_header_above(c(" " = 1, "Census Additions" = 2), background = c("white", sinai_colors[1]), color = "white", line = FALSE)

# Create kable to visualize predicted peak census by site
seir_peak_census_kable <- kable(system_summer_peak, format = "html", escape = FALSE, align = "c", digits = 1,
      col.names =c("Site", "Total COVID Censuus", "Critical Care COVID Census"),
      caption = "Secondary Peak Predictions by Site") %>%
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11, full_width = FALSE) %>%
  column_spec(column = c(1), border_right = "thin solid lightgray", background = "white") %>%
  column_spec(column = c(2:3), background = sinai_colors[1], color = "white", include_thead =  TRUE) %>%

  column_spec(column = c(1:3), background = "inherit", color = "inherit") #%>%
  # add_header_above(c(" " = 1, "Census Predictions" = 2), background = c("white", sinai_colors[1]), color = "white", line = FALSE)
...
2. COVID Steady State Census
    + Based on COVID census from 06/08/20-07/07/20
3. COVID Surge Scenarios
    + Incorporated July/August peak from DOM SEIR model
    + Updated future peak scenarios to reflect 25% and 50% historical peak
    + Projected census for future scenarios updated to reflect difference between steady state COVID census and scenario's peak census
5. Flu Testing
    + Incorporated flu test data for FY2019 from Epic
    + Epic sites: Crosswalked with IP billing data to determine patients with positive flu test results who are ultimately admitted IP or already admitted IP. Utilized this data to quantify impact of these patients to floor and ICU ADC.
...
* Table name: COVID_IP_PATIENT_DAYS_DETAIL pulled on 6/18/20 3:51PM</br>
...
<h4><b>Flu Test Analysis:</b></h4>
**Data Source:**</br>
Based on Epic flu tests ordered during FY2019. Determination of IP admission based on crosswalk with IP discharge billing data.
<h4></h4></br>
...
<h4><b>DOM SEIR Model:</b></h4>
**Data Source:**</br>
Based on "COVID Daily Forecast Model" updated on 06/25. Census and admissions probability outputs provided by Beth Percha on 6/25/2020.
...
<h4><b>ICUs at each site do not always see a drastic increase in census during flu months.</b></h4>
</br>
...
### Flu Test Analysis
#### FY2019 Flu Test Volume (ED and IP)
```{r Call flu volume plot}
flu_test_volume

```

#### FY2019 Positive Flu Test Volume (ED and IP)
```{r Call positive flu test plot}
flu_positive_volume

```


#### Inpatients with Positive Flu Test Results
```{r Call admitted patients with positive flu tests plots}
admitted_flu_positive_volume

```
<br>
*Patients assumed to be admitted if Epic encounter associated with flu test has an inpatient billing charge. Flu test may have been ordered by ED or IP unit.*


```{r Call flu positive inpatient impact to census kable}
flu_positive_census_kable

```
<br>
*Patient days based on total patients days throughout year. ADC based on number of days during flu surge months (November-March).*


### DOM SEIR Model
#### Census Outputs of DOM SEIR Predictive Model
```{r Display census outputs of SEIR model for reference}
seir_system_overall_census

seir_system_icu_census

seir_sec_peak_kable
```
<h4><b>Reintroduction rate and date of 0.017 2020-04-23 used to represent July/August secondary peak. </b></h4>
</br>

#### Probability of Admissions by Site
The average probability of admissions to each site from the SEIR model is used as a scale factor to determine the census peak volume for each site. 
The total census for MSHS is then updated to exclude MSSN for consistency with remainder of model. 
```{r Display probability of admissions breakdown}
seir_adm_prob

adm_prob_kable

seir_peak_census_kable
...
covid_hist_census_peak(site = "MSHS")
...
covid_hist_census_peak(site = "MSH")
...
covid_hist_census_peak(site = "MSQ")
...
covid_hist_census_peak(site = "MSB")
...
covid_hist_census_peak(site = "MSBI")
...
covid_hist_census_peak(site = "MSM")
...
covid_hist_census_peak(site = "MSW")
