...
test <- ip_disch_df %>%
  group_by(Site, `Service Desc Msx`, ServiceInclude, `Unit Desc Msx`, AdultMedSurg, PedsUnit, NurseryNICU) %>%
  summarize(Count = n())
...
```{r Plot ADC during flu season and non-flu season for across sites}
adc_labels <- c(IPADC = "Adult Med Surg", ICUADC = "ICU")

ggplot(data = ms_flu_season_census_site_melt, aes(x = Site, y = value, fill = SeasonType)) +
...
```{r Custom function to plot monthly ADC for each site}
...
# Bind together lowest non-flu month and highest flu month
...
ggplot(data = ms_flu_low_high_months_melt, aes(x = Site, y = value, fill = MonthType)) +
  geom_bar(stat = "identity", position = "dodge") +
...
covid_icu_scenarios_cast <-  dcast(covid_icu_scenarios, Site + CriticalCareCensus ~ ScaleName, value.var = "ScaledICUCensus")
...
covid_census_peak_summary_scenarios <- left_join(covid_census_icu_peak_summary[ , c("Site", "Date", "IPCensus", "ICUCensus", "VentNonICUCensus", "CriticalCareCensus")], covid_icu_scenarios_cast, by = c("Site" = "Site", "CriticalCareCensus" = "CriticalCareCensus"))
...
## Model Updates and Considerations
...
```
**Method 2 provides more conservative critical care census projections and will be used moving forward.**
...
* MSQ 5 East: PACU converted to ICU from 3/28/20-5/6/20
* MSQ 6 East: ICU from 3/28/20-5/22/20 when rooms 601, 606, and 608 converted back to IMCU
