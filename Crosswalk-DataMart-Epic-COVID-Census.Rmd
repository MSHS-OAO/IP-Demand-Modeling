...
#                                     by = c("MRN" = "MRN", "Encounter" = "CSN", "CensusDate" = "CensusDate"))
# 
# 
# reverse_test <- covid_census_crosswalk_reverse %>%
#   filter(is.na(IP_SITE) & CensusDate < as.Date("5/31/20", format = "%m/%d/%y") & GROUP != "ED")
# 
# reverse_test_site <- reverse_test %>%
#   group_by(Site, CensusDate, EpicInfection) %>%
#   summarize(Count = n())
# 
# covid_census_crosswalk_reverse2 <- left_join(epic_covid_census_subset, datamart_covid_census_subset, 
#                                     by = c("Encounter" = "CSN", "CensusDate" = "CensusDate"))
# 
# 
# reverse_test2 <- covid_census_crosswalk_reverse2 %>%
#   filter(is.na(IP_SITE) & CensusDate < as.Date("5/31/20", format = "%m/%d/%y") & GROUP != "ED")
# 
# reverse_test_site2 <- reverse_test2 %>%
#   group_by(Site, CensusDate, EpicInfection) %>%
#   summarize(Count = n())
#   
# reverse_test3 <- covid_census_crosswalk_reverse2 %>%
#   filter(CensusDate < as.Date("5/31/20", format = "%m/%d/%y") & GROUP != "ED") %>%
#   group_by(Site, PATIENT_DAY) %>%
#   summarize(Count = n())
