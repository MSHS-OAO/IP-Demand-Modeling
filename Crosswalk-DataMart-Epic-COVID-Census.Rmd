...
```{r Crosswalk DataMart and Epic data to determine if each census day in DataMart is a COVID day}
covid_census_crosswalk <- left_join(covid_ip_days_df, epic_covid_census_subset,
                                    by = c("MRN" = "MRN", "CSN" = "PAT_ENC_CSN_ID", "CensusDate" = "CensusDate"))

test_match <- covid_census_crosswalk %>%
  filter(COVIDDay == 1)

test_nomatch <- covid_census_crosswalk %>%
  filter(is.na(COVIDDay))

a <- test_match %>%
  group_by(IP_SITE, CENSUS_DEPT, DEPARTMENT_NAME) %>%
  summarize(Count = n())
...
# Compare ICU census using 2 methods - date with peak IP COVID census and date with peak ICU COVID census
compare_covid_census_peak_site <- ip_covid_daily_census_site %>%
  group_by(IP_SITE) %>%
...
compare_covid_census_peak_system <- ip_covid_daily_census_system %>%
  group_by(IP_SITE = "MSHS") %>%
  summarize(IPPeakDate = CensusDate[which.max(IPCOVIDCensus)],
            IPPeakIPCensus = IPCOVIDCensus[which.max(IPCOVIDCensus)],
            IPPeakICUCensus = ICUCOVIDCensus[which.max(IPCOVIDCensus)],
            ICUPeakDate = CensusDate[which.max(ICUCOVIDCensus)],
...
compare_covid_peak_summary <- rbind(compare_covid_census_peak_site, compare_covid_census_peak_system)

## Moving forward, use peak ICU census day for conservative analysis

# Create dataframes with peak ICU census at each site and across the system
covid_census_icu_peak_site <- ip_covid_daily_census_site %>%
  group_by(IP_SITE) %>%
  summarize(ICUPeakDate = CensusDate[which.max(ICUCOVIDCensus)],
            IPCensus = IPCOVIDCensus[which.max(ICUCOVIDCensus)], 
            ICUCensus = ICUCOVIDCensus[which.max(ICUCOVIDCensus)],
            NonICUVentCensus = NonICUVentCensus[which.max(ICUCOVIDCensus)],
            CriticalCareCensus = CriticalCareCensus[which.max(ICUCOVIDCensus)],
            NonCriticalCareCensus = NonCriticalCareCensus[which.max(ICUCOVIDCensus)])
...
covid_census_steady_state_site <- ip_covid_daily_census_site %>%
  filter(CensusDate >= covid_steady_state_start & CensusDate <= covid_steady_state_end) %>%
  group_by(IP_SITE) %>%
  summarize(AvgIPCensus = mean(IPCOVIDCensus),
            AvgICUCensus = mean(ICUCOVIDCensus),
            AvgNonICUVentCensus = mean(NonICUVentCensus),
            AvgCriticalCareCensus = mean(CriticalCareCensus))

covid_census_steady_state_system <- ip_covid_daily_census_system %>%
  filter(CensusDate >= covid_steady_state_start & CensusDate <= covid_steady_state_end) %>%
  summarize(IP_SITE = "MSHS", 
            AvgIPCensus = mean(IPCOVIDCensus),
            AvgICUCensus = mean(ICUCOVIDCensus),
            AvgNonICUVentCensus = mean(NonICUVentCensus),
            AvgCriticalCareCensus = mean(CriticalCareCensus))

covid_census_steady_state_summary <- rbind(covid_census_steady_state_site, covid_census_steady_state_system)
```
