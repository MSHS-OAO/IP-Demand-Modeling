...
epic_unit_mappings <- read_xlsx(path = paste0(user_directory, "/Epic Census Trending/Epic-Oracle-Census-Unit-Mappings-2020-08-05.xlsx"), sheet = "UnitMappings", na = c("", "NA"))
...
diveport_unit_mappings <- read_excel(paste0(user_directory, "/Ad Hoc Analysis Outputs for AZ/DivePort_Census_Unit_Reference_2020-08-28.xlsx"), na = c("", NA))
...
epic_ip_census_df <- oracle_ip_census_df
...
epic_ip_census_df <- epic_ip_census_df %>%
  filter(IP_SITE != "MSBI") %>%
...
epic_ip_census_df <- left_join(epic_ip_census_df, epic_unit_mappings,
...
all_sites_adult_census <- rbind(epic_site_adult_census_summary, msbi_census_summary)

# Summarize census across all sites to determine system total daily census
system_adult_census_summary <- all_sites_adult_census %>%
  group_by(CensusDate) %>%
...
# Reorder columns
system_adult_census_summary <- system_adult_census_summary[ , colnames(all_sites_adult_census)]

# Bind site and system census together
site_system_census_summary <- rbind(all_sites_adult_census, system_adult_census_summary)

# Set census sites as factor for sorting
site_system_census_summary$CensusSite <- factor(site_system_census_summary$CensusSite, 
                                          levels = site_order,
                                          ordered = TRUE)
...
peak_census_summary <- site_system_census_summary %>%
  group_by(CensusSite) %>%
...
peak_census_kable <- kable(peak_census_summary, format = "html", escape = FALSE, align = "c", digits = 0,
                           ...
                           caption = paste0("Peak IP Census During COVID Surge (",
                                            ...
                                            format(max(site_system_census_summary$CensusDate), "%m/%d/%y"),
                                            ")")) %>%
...
epic_unit_mappings_kable <- kable(epic_unit_mappings_reference, format = "html", escape = FALSE, align = "c", digits = 0,
...
dp_unit_mappings_kable <- kable(dp_unit_mappings_reference, format = "html", escape = FALSE, align = "c", digits = 0,
                             col.names =c("Unit", "Unit Type", "Include/Exclude from Analysis"),
                             caption = "MSBI Unit Classification and Inclusion Criteria") %>%
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11, full_width = FALSE) %>%
  column_spec(column = c(1:3), border_right = "thin solid lightgray") %>%
  column_spec(column = c(1:3), background = sinai_colors[1], color = "white", include_thead = TRUE) %>%
  column_spec(column = c(1:3), background = "inherit", color = "inherit")
