...
```{r Test code for plotting data in different ways}
...
```{r Plot data for a given site}
site1 <- "MSH"

site_df <- census_site_unit_type %>%
  filter(CensusSite == site1 & CensusDate > start_date)
...
for(i in 1:length(unit_types)) {
  ...
  # Plot census for all units in this unit type
  site_unit_type_plot <- ggplot(data = unit_type_df,
                           aes(x = CensusDate, y = Census, color = CENSUS_DEPT)) + #, shape = CENSUS_DEPT)) +
    geom_line() +
    geom_point() +
    labs(title = paste0(site1, " ", unit_type, " Census by Unit"), x = "Date", y = "Census") +
    theme_bw() +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          legend.position = "bottom",
          legend.box = "horizontal",
...
```{r Plot MSH census trends}
site_unit_type_plotting(site = "MSH")

```

```{r Plot MSQ census trends}
site_unit_type_plotting(site = "MSQ")

```

```{r Plot MSB census trends}
site_unit_type_plotting(site = "MSB")

```

```{r Plot MSM census trends}
site_unit_type_plotting(site = "MSM")

```

```{r Plot MSW census trends}
