...
wave1_ip_census <- tbl(con, "EMR_ALL_PAT_DAYS_MSX") %>%
...
wave2_ip_census <- tbl(con, "EMR_ALL_PAT_DAYS_MSX") %>%
...
epic_unit_mappings <- read_xlsx(path = paste0(
  ...
  "Epic-Oracle-Census-Unit-Mappings-2021-03-15.xlsx"),
...
```{r Import MSBI DivePort census data and unit mppings}
...
wave1_start_date <- as.Date("3/16/20", format = "%m/%d/%y")

wave2_start_date <- as.Date("12/1/20", format = "%m/%d/%y")

wave1_peak_covid_date <- as.Date("4/9/20", format = "%m/%d/%y")

wave2_peak_covid_date <- as.Date("1/18/21", format = "%m/%d/%y")

```


```{r Format Epic raw census data}
epic_census_wave1 <- wave1_ip_census
epic_census_wave2 <- wave2_ip_census
...
epic_census_wave1 <- epic_census_wave1 %>%
  filter(IP_SITE != "MSBI") %>%
...
epic_census_wave1 <- left_join(epic_census_wave1, epic_unit_mappings,
                          by = c("CENSUS_DEPT" = "CENSUS_DEPT"))

# Format census date and determine if the census date is an ICU day
epic_census_wave2 <- epic_census_wave2 %>%
  mutate(CensusDate = date(CENSUS_DATE), 
         ICUDay = (IP_SITE != "MSQ" & ACCOMMODATION_DESC %in%
                     c("INTENSIVE CARE", "CORONARY CARE")) |
           CENSUS_DEPT == "MSQ 6 ICU" | 
...
# Crosswalk census data with unit mappings
epic_census_wave2 <- left_join(epic_census_wave2, epic_unit_mappings,
                          by = c("CENSUS_DEPT" = "CENSUS_DEPT"))
...
epic_adult_census_wave1 <- epic_census_wave1 %>%
  filter(CensusDate >= wave1_start_date, 
         UnitType %in% c("Adult ICU", "Adult MS", "COVID Surge") &
           PATIENT_DAY == 1)

epic_adult_census_wave2 <- epic_census_wave2 %>%
  filter(CensusDate >= wave2_start_date, 
         UnitType %in% c("Adult ICU", "Adult MS", "COVID Surge") &
           PATIENT_DAY == 1)

# Summarize census by day for each site
epic_site_adult_census_wave1_summary <- epic_adult_census_wave1 %>%
...
epic_site_adult_census_wave2_summary <- epic_adult_census_wave2 %>%
  group_by(CensusSite, CensusDate) %>%
  summarize(Adult_Census = sum(PATIENT_DAY),
            CC_Census = sum(CriticalCareDay, na.rm = TRUE),
            MS_Census = Adult_Census - CC_Census,
...
msbi_census_df <- left_join(msbi_census_df, diveport_unit_mappings[ , c("MSBI DivePort Units", "Include/Exclude", "Unit Type")], by = c("Unit" = "MSBI DivePort Units"))
...
msbi_census_summary <- msbi_census_summary[ , c("CensusSite", "CensusDate", 
                                                "Adult_Census", "CC_Census", "MS_Census")]
...
all_sites_census_wave1 <- rbind(epic_site_adult_census_wave1_summary, msbi_census_summary)

# Summarize census across all sites to determine system total daily census
system_census_wave1_summary <- all_sites_census_wave1 %>%
  group_by(CensusDate) %>%
...
system_census_wave2_summary <- epic_site_adult_census_wave2_summary %>%
  group_by(CensusDate) %>%
  summarize(CensusSite = "MSHS",
            Adult_Census = sum(Adult_Census),
            CC_Census = sum(CC_Census),
...
# Reorder columns
system_census_wave1_summary <-
  system_census_wave1_summary[, colnames(all_sites_census_wave1)]

system_census_wave2_summary <-
  system_census_wave2_summary[, colnames(epic_site_adult_census_wave2_summary)]


# Bind site and system census together
site_system_census_wave1_summary <- rbind(all_sites_census_wave1,
                                          system_census_wave1_summary)

site_system_census_wave2_summary <- rbind(epic_site_adult_census_wave2_summary,
                                          system_census_wave2_summary)

# Set census sites as factor for sorting
site_system_census_wave1_summary <- site_system_census_wave1_summary %>%
  mutate(CensusSite = factor(CensusSite,
                             levels = site_order,
                             ordered = TRUE))

site_system_census_wave2_summary <- site_system_census_wave2_summary %>%
  mutate(CensusSite = factor(CensusSite,
                             levels = site_order,
                             ordered = TRUE))
...
peak_census_wave1_summary <- site_system_census_wave1_summary %>%
  group_by(CensusSite) %>%
...
peak_census_wave1_kable <- 
  kable(peak_census_wave1_summary, format = "html", escape = FALSE, align = "c",
        digits = 0,
        col.names =c("Site",
                     "Date", "Census",
                     "Date", "Census",
        ...
        caption = paste0("Peak IP Census During First COVID Surge (",
                         format(wave1_start_date, "%m/%d/%y"),
                         " - ",
                         format(
                           max(site_system_census_wave1_summary$CensusDate),
                           "%m/%d/%y"),
                         ")")) %>%
  ...
  row_spec(row = nrow(peak_census_wave1_summary),
...
census_covid_peak_wave1 <- site_system_census_wave1_summary %>%
  filter(CensusDate == wave1_peak_covid_date) %>%
  arrange(CensusSite)

census_covid_peak_wave1_kable <-
  kable(census_covid_peak_wave1, format = "html", escape = FALSE, align = "c",
        digits = 0,
        col.names =c("Site",
                     "Date",
                     "Census",
        ...
        caption = paste0("IP Census During First Surge COVID Peak On ",
                         format(wave1_peak_covid_date, "%m/%d/%y"))) %>%
  kable_styling(bootstrap_options = "hover", position = "center",
                font_size = 11, full_width = FALSE) %>%
  ...
  # row_spec(row = 0, background = "#221f72", color = "white") %>%
  row_spec(row = nrow(census_covid_peak_wave1), background = "lightgrey",
           bold = T) %>%
...
# Create dataframe with peak total census, critical care census, and med-surg census
peak_census_wave2_summary <- site_system_census_wave2_summary %>%
  group_by(CensusSite) %>%
  summarize(PeakAdultCensusDate = CensusDate[which.max(Adult_Census)], 
            PeakAdultCensus = Adult_Census[which.max(Adult_Census)],
            PeakCCCensusDate = CensusDate[which.max(CC_Census)],
...
# Create kable to visualize results
peak_census_wave2_kable <- 
  kable(peak_census_wave2_summary, format = "html", escape = FALSE, align = "c",
        digits = 0,
        col.names =c("Site",
                     "Date", "Census",
                     "Date", "Census",
        ...
        caption = paste0("Peak IP Census During Second COVID Surge (",
                         format(wave2_start_date, "%m/%d/%y"),
                         " - ",
                         format(
                           max(site_system_census_wave2_summary$CensusDate),
                           "%m/%d/%y"),
                         ")")) %>%
  kable_styling(bootstrap_options = "hover", position = "center",
                font_size = 11, full_width = FALSE) %>%
  ...
  column_spec(column = c(2:7), background = "inherit", color = "inherit") %>%
  # row_spec(row = 0, background = "#221f72", color = "white") %>%
  row_spec(row = nrow(peak_census_wave2_summary),
           background = "lightgrey",
           bold = T) %>%
  add_header_above(c(" " = 1,
                     "Total Adult Census" = 2,
...
census_covid_peak_wave2 <- site_system_census_wave2_summary %>%
  filter(CensusDate == wave2_peak_covid_date) %>%
  arrange(CensusSite)

census_covid_peak_wave2_kable <-
  kable(census_covid_peak_wave2, format = "html", escape = FALSE, align = "c",
        digits = 0,
        col.names =c("Site",
                     "Date",
                     "Census",
        ...
        caption = paste0("IP Census During Second Surge COVID Peak On ",
                         format(wave2_peak_covid_date, "%m/%d/%y"))) %>%
  kable_styling(bootstrap_options = "hover", position = "center",
                font_size = 11, full_width = FALSE) %>%
  column_spec(column = c(1:5), border_right = "thin solid lightgray") %>%
  column_spec(column = c(3), background = sinai_colors[1],
  ...
  column_spec(column = c(3:5), background = "inherit", color = "inherit") %>%  
  # row_spec(row = 0, background = "#221f72", color = "white") %>%
  row_spec(row = nrow(census_covid_peak_wave2), background = "lightgrey",
           bold = T) %>%
  add_header_above(c(" " = 2,
                     "Total Adult Census" = 1,
                     "Adult Critical Care Census" = 1,
...
epic_unit_mappings_reference$CensusSite <- factor(epic_unit_mappings_reference$CensusSite,
                                             levels = site_order,
                                             ordered = TRUE)

epic_unit_mappings_reference$UnitRollUp <- factor(epic_unit_mappings_reference$UnitRollUp,
                                             levels = unit_rollup_order, 
                                             ordered = TRUE)

epic_unit_mappings_reference <- epic_unit_mappings_reference %>%
...
dp_unit_mappings_reference$InclExcl <- factor(dp_unit_mappings_reference$InclExcl, 
                                                       levels = dp_criteria, ordered = TRUE)

dp_unit_mappings_reference$UnitType <- factor(dp_unit_mappings_reference$UnitType,
                                              levels = dp_unit_types, ordered = TRUE)
...
2. MSBI: Analysis based on DivePort midnight census data for March and April 2020. Data pulled on 08/27/2020.
<h4></h4></br>
