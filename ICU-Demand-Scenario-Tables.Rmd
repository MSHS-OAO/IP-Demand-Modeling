...
covid_ip_days_df_raw <- read.csv("COVID_IP_PATIENT_DAYS_DETAIL_202005121020.csv", stringsAsFactors = FALSE, na = c("", "NA"))

# Import FY2019 IP discharge data
setwd("..\\IP Discharge Data")

ip_disch_df_raw <- read_excel("IP Disch FY2019 2020-05-13.xlsx", na = c("", "NA"),
                                col_types = c("text", "text", "text", "text", "date", "date", "date",
                                              "text", "text", "text", "text", "text", "text", "text",
                                              "numeric", "numeric"))

# Import analysis reference
reference_file <- "Analysis Reference 2020-05-19.xlsx"
...
# MSSN & MSQ: Any bed type defined as critical care or critical care surge
# Other sites: Accommodation code of intensive care or coronary care
# COVID cohort COVID_FLAGS: Positive, PUI, Susc, and Resolved
covid_ip_days_df <- covid_ip_days_df %>%
  mutate(ICU = ((IP_SITE == "MSSN" | IP_SITE == "MSQ") & (CENSUS_BED_TYPE %in% "Critical Care" | CENSUS_BED_TYPE %in% "Critical Care Surge")) |
           (IP_SITE != "MSSN" & IP_SITE != "MSQ" & (ACCOMMODATION_DESC %in% "INTENSIVE CARE" | ACCOMMODATION_DESC %in% "CORONARY CARE")),
...
ip_covid_daily_census_site <- covid_ip_days_df %>%
  ...
  summarize(IPCOVIDCensus = n(), ICUCOVIDCensus = sum(ICU), NonICUCOVIDCensus = sum(!ICU))
...
covid_census_peak_site <- ip_covid_daily_census_site %>%
  group_by(IP_SITE) %>%
  summarize(PeakDate = CensusDate[which.max(IPCOVIDCensus)],
            PeakIPCensus = IPCOVIDCensus[which.max(IPCOVIDCensus)],
            ICUCensus = ICUCOVIDCensus[which.max(IPCOVIDCensus)],
            NonICUCOVIDCensus = NonICUCOVIDCensus[which.max(IPCOVIDCensus)])


covid_census_peak_system <- ip_covid_daily_census_system %>%
  summarize(IP_SITE = "MSHS",
            PeakDate = CensusDate[which.max(IPCOVIDCensus)],
            PeakIPCensus = IPCOVIDCensus[which.max(IPCOVIDCensus)],
            ICUCensus = ICUCOVIDCensus[which.max(IPCOVIDCensus)],
            NonICUCOVIDCensus = NonICUCOVIDCensus[which.max(IPCOVIDCensus)])

covid_census_peak_summary <- rbind(covid_census_peak_site, covid_census_peak_system)
```
...
ip_disch_subset <- ip_disch_df %>%
  filter(NurseryNICU == "No" & AdmitType != "Newborn" & (`Icu Days Msx` > 0 | AdultMedSurg == "Yes"))
...
covid_icu_scenarios <- covid_census_peak_summary %>%
