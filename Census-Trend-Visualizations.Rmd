...
#### Census data available through `r format(Sys.Date() - 1, "%m/%d/%y")` 11:59PM
...
unit_mappings_reference <- unit_mapping %>%
...
Analysis based on Epic midnight census data. Data obtained from Oracle database table "EMR_EMR_ALL_PAT_DAYS_MSX".
<h4></h4></br>


<h4><b>Notes on Go-Live Dates:</b></h4>

1. MSB Epic inpatient go-live date of 9/8/2019. All inpatient data for MSB prior to this date is excluded.
    + ADC for MSB for September 2019 takes this start date into account.
2. MSBI Epic inpatient go-live did not occur until August 2020 and is not included in this analysis as a result.


<h4><b>Analysis Methodology:</b></h4>
All inpatient units are classified in one of the categories listed below:

1. Adult ICU
2. Adult Med Surg
3. COVID Surge
4. Peds & Peds ICU
5. L&D and Postpartum
6. Nursery & NICU
7. Psych & Rehab

ED, Psych ED, and non-inpatient units are excluded from analysis. 

The table below summarizes the unit mappings for each site.

```{r Display unit mappings table}
...
<!-- #   num_units <- length(unique(data$CENSUS_DEPT)) -->
<!-- #  -->
<!-- #   census_trend_plot <- ggplot(data = data, aes(x = CensusDate, y = Census, color = CENSUS_DEPT)) + -->
<!-- #     geom_line() + -->
<!-- #     geom_point() + -->
<!-- #     labs(title = title, x = "Date", y = "Census") + -->
<!-- #     theme_bw() + -->
...
<!-- #  -->
<!-- # # Determine unit types for this site -->
<!-- # unit_types <- as.vector(unique(site_df$UnitRollUp)) -->
<!-- #  -->
<!-- # test <- lapply(unit_types, function(x) { -->
<!-- #    -->
<!-- #   cat("\n##", x, "\n") -->
<!-- #   plot_df <- site_df %>% -->
<!-- #     filter(UnitRollUp == x) -->
...
<!-- ```{r Test replacing function with multiple nested lapply statements, echo = FALSE, results = 'asis'} -->
<!-- # -->
<!-- # sites <- unique(census_site_unit_type$CensusSite) -->
<!-- # -->
...
<!-- #   # Subset census data based on site and start date -->
<!-- #   site_df <- census_site_unit_type %>% -->
<!-- #     filter(CensusSite == x & CensusDate > start_date) -->
<!-- # -->
<!-- #   # Determine unit types for this site -->
<!-- #   unit_types <- as.vector(unique(site_df$UnitRollUp)) -->
<!-- # -->
<!-- #   # Iterate through unit types for this site -->
<!-- #   lapply(unit_types, function(y) { -->
<!-- # -->
<!-- #     cat(" \n###", y, "\n") -->
<!-- # -->
<!-- #     plot_df <- site_df %>% -->
...
<!-- # -->
<!-- #     if (num_units <= max_num_units) { -->
<!-- #       plot_title <- paste(x, y, "Census by Unit") -->
<!-- # -->
<!-- #       unit_type_census_plot2(data = plot_df, title = plot_title) -->
<!-- # -->
<!-- #     } else { -->
...
<!-- #         groupings <- seq(1, max(plot_df$Grouping), 1) -->
<!-- # -->
<!-- #         # Iterate through unit groupings if the number of units exceeds the desired number of curves on the graph -->
<!-- #         lapply(groupings, function(z) { -->
<!-- # -->
<!-- #           grouping_df <- plot_df %>% -->
<!-- #             filter(Grouping == z) -->
<!-- # -->
<!-- #           plot_title <- paste0(x, " ", y, " Census by Unit (", z, " of ", max(groupings), ")") -->
<!-- # -->
<!-- #           unit_type_census_plot2(data = grouping_df, title = plot_title) -->
<!-- # -->
<!-- #           }) -->
