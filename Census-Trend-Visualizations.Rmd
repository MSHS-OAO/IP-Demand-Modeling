...
# Summarize adult census (ICU, Med Surg, COVID Surge)
...
colnames(census_monthly_comp_cast)[4:ncol(census_monthly_comp_cast)] <- format(as.Date(colnames(census_monthly_comp_cast)[4:ncol(census_monthly_comp_cast)],
                                                                                       "%Y-%m-%d"), "%b-%y")
# # Subset data based on site and unit type and remove unused columns for kable
# site <- "MSB"
# unittype <- "Med Surg"
# 
# census_month_comp_kable <- census_monthly_comp_cast %>%
#   filter(CensusSite == site & UnitRollUp == unittype)
# 
# # census_month_comp_kable$CensusSite <- NULL
# # census_month_comp_kable$UnitRollUp <- NULL
# 
# 
# kable(census_month_comp_kable, format = "html", escape = FALSE, align = "c", digits = 1,
#       caption = "MSB COVID Surge ADC Change - 2019 vs. 2020 for Last 3 Months") %>%
...
# census_compare_kable(census_month_comp_kable)

## Issues:
## 1. some sites have had new units open in 2020 that didn't exist in 2019 --> solution: maybe cast data before loop? Resolved
...
```{r Iterate through data to create kables for each site and unit type}
site_unit_type_kable <- function(site) {
  ...
  unit_types <- unique(site_census_comp_kable$UnitRollUp)
  ...
  for(i in 1:length(unit_types)){
    ...
    kable_title <- paste(site, unit_types[i], " ADC Change - 2019 vs. 2020 for Last 3 Months")
...
site_census_summary_plot <- function(site) {
  ...
  ggplot(data = site_adult_census_plot_df, 
    ...
    geom_line(color = sinai_colors[1]) +
...
# site_census_summary_plot(site = "MSH")
site_unit_type_plotting(site = "MSH")
# site_unit_type_kable(site = "MSH")
